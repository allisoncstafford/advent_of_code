...
class IntcodeComp():
    ...
    def compute(self, input_signal=None):
        ...
        while (True):
            ...
            # print(f"opcode: {opcode}  current: {self.current}  relative base: {self.relative_base}")
            # print(self.code[:101])
            if opcode == '99':
                self.stop()
            ...
            # print(f"values: {values}  target_loc: {target_loc}")
            if opcode == '01':
                ...
                if output1 == None:
                    output1 = get_output(self.code, target_loc)
                else:
                    output2 = get_output(self.code, target_loc)
                    return output1, output2, self.stopped
            elif opcode == '05':
